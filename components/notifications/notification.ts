...
const categorizeNotification = (createdAt: string): { category: 'today' | 'thisWeek' | 'thisMonth', time: string } => {
  const notificationDate = parseISO(createdAt);
  const timeDifference = formatDistanceToNow(notificationDate, { addSuffix: true });

  const today = new Date().setHours(0, 0, 0, 0);
  const thisWeek = new Date();
  thisWeek.setDate(thisWeek.getDate() - 7);

  if (notificationDate.getTime() >= today) return { category: "today", time: timeDifference };
  if (notificationDate >= thisWeek) return { category: "thisWeek", time: timeDifference };
  return { category: "thisMonth", time: timeDifference };
};

export const transformNotifications = (notifications: any[]) => {
  const groupedNotifications: { today: any[], thisWeek: any[], thisMonth: any[] } = {
    today: [],
    thisWeek: [],
    thisMonth: [],
  };
  ...
  sortedNotifications?.forEach((notification: any) => {
    ...
    const { category, time }: { category: 'today' | 'thisWeek' | 'thisMonth', time: string } = categorizeNotification(createdAt);
