...
import { addQuizToModuleService, updateQuizQuestionService, addQuestionToQuizService } from "@/services/quiz.service"
import type { QuestionData, QuestionType, QuizData } from "@/types"
...
const Quiz = ({ courseId: id }: { courseId: any }) => {
  ...
  // Function to refresh quiz data
  const refreshQuizData = () => {
    if (selectedModule?.id) {
      queryClient.invalidateQueries({ queryKey: ["quizData", selectedModule?.id] })
  ...
  // Add mutation for adding a new question to an existing quiz
  const { mutate: handleAddQuestionToQuiz, isPending: isAddingQuestion } = useMutation({
    mutationFn: (payload: any) => addQuestionToQuizService(currentQuizId || 1, payload),
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast.error(error?.data?.[0] || "Failed to add new question")
    },
  })
  ...
  const areQuestionsComplete = () => {
    return questionData.every((data, index) => {
      ...
      if (question.type === "trueFalse") {
        ...
        return data.questionText.trim() && data.optionValues.every((opt) => opt.trim()) && data.selectedOption !== null
      }
    })
  }
  ...
  const handleSubmitChanges = async () => {
    ...
    if (editingQuestionIndex !== null && questionData[0]?.questionId) {
      const formattedQuestions = questionData.map((data, index) => {
        const question = questions[index]
        if (question.type === "multipleChoice") {
          const options = data.optionValues.map((text, optIndex) => ({
  ...
  // New function to handle submitting a new question to an existing quiz
  const handleAddNewQuestion = () => {
    if (questionData.length === 0 || !currentQuizId) return
    ...
    console.log("Adding new question to quiz:", currentQuizId, formattedQuestion)

    handleAddQuestionToQuiz(formattedQuestion)
  }

  if (isUpdatingQuizQuestion || isAddingQuestion) {
    return <RouterSpinner />
  }

  const isSubmitDisabled = !quizTitle.trim() || questions.length === 0 || !areQuestionsComplete() || isAddingModule
  const isQuestionComplete = areQuestionsComplete() && questions.length > 0
              ...
              {!isAddingQuiz ? (
                ...
                <form onSubmit={handleQuizSubmit}>
                  ...
                  <div className="flex gap-4 mt-4">
                    {showQuestionOptions ? (
                      <>
                        <Button onClick={() => addQuestion("multipleChoice")} variant="outline" type="button">
                          Multiple Choice
                        </Button>
                        <Button onClick={() => addQuestion("trueFalse")} variant="outline" type="button">
                          True/False
                        </Button>
                      </>
                    ) : (
                    ...
                    {/* Show Submit Quiz button only when creating a new quiz */}
                    {!isAddingNewQuestion && !editingQuestionIndex && (
                    ...
                    {/* Show Submit Edit button when editing an existing question */}
                    ...
                    {/* Show Submit New Question button when adding a new question to an existing quiz */}
                    {isAddingNewQuestion && !editingQuestionIndex && (
                      <Button
                        onClick={handleAddNewQuestion}
                        type="button"
                        disabled={!isQuestionComplete || isAddingQuestion}
                      >
                        {isAddingQuestion ? (
                          <p className="flex items-center gap-2">
                            <Loader2 className="animate-spin" /> Please wait...
