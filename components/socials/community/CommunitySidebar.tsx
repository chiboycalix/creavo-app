...
export default function CommunitySidebar() {
  ...
  const [selectedCommunity, setSelectedCommunity] = useState<{ value: string, label: string, logo: string, spaces: [] }>({
    value: "",
    label: "",
    logo: "",
    spaces: []
  ...
  console.log(selectedCommunity, "selectedCommunityselectedCommunity")
  return (
    <Card className='rounded-md h-[87vh] bg-white'>
      {
        isFetchingCommunity || loading || profileLoading ? <CommunitySidebarSkeleton /> :
          <div className="w-64 p-4">
            <div className='flex items-center space-x-2 pt-2 pb-4 mb-6 border-b'>
              <CommunityList communities={communities && communities?.data?.communities} setSelectedCommunity={setSelectedCommunity} />
            </div>
                ...
                <CollapsibleContent className="py-2">
                  {
                    selectedCommunity.label !== "" ?
                      selectedCommunity?.spaces?.map((space: any) => {
